...
const initializeAutocomplete = async () => {
    const activeTab = tabListHeader?.querySelector('.active');
...
const paymentLink = async () => {
    ...
    if (activeTab?.dataset.menuItem === 'TP_REMITTANCES') {
        ...
        const orderScope = orderElement ? angular.element(orderElement).scope() : null;
        const paymentScope = paymentElement ? angular.element(paymentElement).scope() : null;
        const contactScope = contactElement ? angular.element(contactElement).scope() : null;
        const customerScope = customerElement ? angular.element(customerElement).scope() : null;
        const orderRecord = orderScope?.record;
        const paymentRecord = paymentScope?.record;
        const contactRecord = contactScope?.record;
        ...
        const copyToClipboard = () => {
            navigator.clipboard.writeText(linkTextArea.value).then(() => console.log('Copied to clipboard'), (err) => console.error('Failed to copy text:', err));
        };
