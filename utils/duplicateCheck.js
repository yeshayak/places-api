...
export const duplicateCheck = async (lookupName, root = angular.element('#contextWindow').scope(), customerIdKey = 'TABPAGE_1.order') => {
  if (!root) {
  ...
  const customerId = root.windowData[customerIdKey]?.[0]?.customer_id;
